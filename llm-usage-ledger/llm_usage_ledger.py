#!/usr/bin/env python3
"""
llm-usage-ledger: org-wide LLM usage accounting — every stream, one ledger.

Global skill, usable from ANY repo (and by non-Claude tools — plain stdlib
Python, no pip dependencies). Tracks EVERY LLM usage stream in one
append-only, cross-repo token ledger:

  1. Cloud API calls: instrument any repo's client with log_usage() (3 lines).
  2. Subscription tools (Claude Code, Codex): `ingest` derives ledger events
     from local transcripts — flat-fee usage priced at API rates as "what
     the plan absorbed".
  3. Local models (LM Studio etc.): the lmstudio-panel skill delegates its
     logging here; no local-model software is required to use this skill.
...
analyses need no re-instrumentation. Prices are an append-only dated series
joined as-of each event's date, so historical reports stay reproducible.

CLI:
  llm_usage_ledger.py report [--by project|model|user] [--windows] [--days N]
  llm_usage_ledger.py report --html [PATH]      # interactive $-vs-tokens dashboard
  llm_usage_ledger.py prices update | list      # dated hosted-rate series
  llm_usage_ledger.py ingest claude-code|codex  # subscription transcripts
"""
...
def ledger_file() -> Path:
    # Filename prefix stays "lmstudio-" (this code's original home was the
    # lmstudio-panel skill) so existing ledger data keeps merging seamlessly;
    # the per-event `provider` field is what distinguishes streams.
    return ledger_dir() / f"lmstudio-{whoami()}-{machine_name()}.jsonl"


def log_usage(model: str, usage: dict, duration_s: float,
              task_tag: str = None, project: str = None,
              provider: str = "lmstudio") -> dict:
    ...
    try:
        ...
        print(f"llm-usage-ledger: ledger write failed ({e!r})", file=sys.stderr)
    return event
...
def print_report(by="project-model", windows=False, days=None):
    ...
    print(f"llm-usage-ledger report ({span}, {len(events)} calls, "
